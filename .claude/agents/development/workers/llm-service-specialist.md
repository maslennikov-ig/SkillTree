...
1. **OpenAI SDK Patterns**:
   ```markdown
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Use mcp__context7__resolve-library-id: "openai"
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Then mcp__context7__get-library-docs with topic: "chat completions"
   Validate: API structure, retry logic, error handling
   ```

2. **Error Handling**:
   ```markdown
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Use mcp__context7__get-library-docs with topic: "error handling"
