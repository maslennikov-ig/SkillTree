...
1. **Primary**: Docs L1/L2 MCP for Jina AI and similarity documentation
...
2. **For Semantic Similarity**:
   ```markdown
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Use mcp__context7__resolve-library-id: "jina-ai"
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Then mcp__context7__get-library-docs with topic: "semantic similarity"
   Validate: Cosine similarity computation, quality thresholds, industry standards
   ```

3. **For Quality Metrics**:
   ```markdown
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Use mcp__context7__get-library-docs with topic: "quality metrics"
