...
1. **Primary**: Docs L1/L2 MCP for Qdrant/Jina documentation
...
1. **For Collection Issues**:
   ```markdown
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Use mcp__context7__resolve-library-id: "qdrant-js"
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Then mcp__context7__get-library-docs with topic: "collections"
...
2. **For Query Issues**:
   ```markdown
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Use mcp__context7__resolve-library-id: "qdrant-js"
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Then mcp__context7__get-library-docs with topic: "filters"
   Validate: filter syntax, payload field matching
   ```

3. **For Embedding Issues**:
   ```markdown
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Use mcp__context7__resolve-library-id: "jina-ai"
   // L2 fallback only when @neuledge/context is missing/stale/insufficient: Then mcp__context7__get-library-docs with topic: "embeddings"
