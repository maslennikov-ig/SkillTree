...
- `resolve-library-id({libraryName})` → `get-library-docs({context7CompatibleLibraryID, topic})`: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient
...
1. **Documentation Research Strategy** (MANDATORY FIRST STEP):
   ...
   **Tier 1: Docs L1/L2 MCP (MANDATORY) - Use After Project Search**
   - **What**: `resolve-library-id({libraryName})` → `get-library-docs({context7CompatibleLibraryID, topic})`: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient
...
2. **Use Docs L1/L2 for validation** (MANDATORY):
   ...
   - Example:
     ...
     // L2 fallback only when @neuledge/context is missing/stale/insufficient: mcp__context7__get-library-docs({
       context7CompatibleLibraryID: "/facebook/react",: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient
...
9. **Documentation References** (MANDATORY):
   ...
   - **Tier 1**: Docs L1/L2 MCP findings with direct quotes
...
- ✅ **MANDATORY: Include direct quotes from Docs L1/L2 MCP documentation**
